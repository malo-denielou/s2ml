...
let check_no_blind_fork (role_assoc:node_to_role) (g:graph) =
  let rec check_width origin (visi,visj) roles i j (sli,slj) = 
    match (sli,slj) with
      ...
      | (i,m,k)::q,(j,n,l)::r -> let _ = debug_prop 
          (Printf.sprintf "Prop 3: Width =>from %d,%d, looking at %s %d, %s %d" i j
             m.label k n.label l)
        in
        ...
        (if (List.mem rolek roles) || (List.mem rolel roles) 
         ...
         then
           (rolei = origin || rolej = origin
               || (List.mem (i,m,k) visi) || (List.mem (j,n,l) visj)
        ...
        && (let _ = debug_prop 
            ...
            rolej = origin || (List.mem (j,n,l) visj) || check_width origin (visi,(j,n,l)::visj) (roles) i j (sli,r))
        && (let _ = debug_prop 
              (Printf.sprintf "Prop 3: from %d,%d, finished %d,%d, absorbing %d" i j k l i) in
            rolei = origin ||  (List.mem (i,m,k) visi) || check_width origin ((i,m,k)::visi,visj) (roles) i j (q,[(j,n,k)]))
          (* Optimizing here : it was (q,slj) *)
  and check_depth origin visited roles i j =
    ...
    if ((List.mem rolei roles) || (List.mem rolej roles) 
        || (rolei = rolej))
    then 
      (check_width origin visited (remove_duplicates (rolei::rolej::roles)) i j (sli,slj))
    else begin
...
let check_all (role_assoc:node_to_role) (g:graph) =
  ...
  let _ = (check_no_blind_fork role_assoc g) in
